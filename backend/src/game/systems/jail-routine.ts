...
const LOCKDOWN_CELL_AREAS = [
  { zoneId: 'cell_area_01', capacity: 2 }, // Ground floor
  { zoneId: 'cell_area_02', capacity: 3 },
  { zoneId: 'cell_area_03', capacity: 2 },
  { zoneId: 'cell_area_04', capacity: 3 },
  { zoneId: 'cell_area_05', capacity: 2 }, // First floor
  { zoneId: 'cell_area_06', capacity: 3 },
  { zoneId: 'cell_area_07', capacity: 2 },
  { zoneId: 'cell_area_08', capacity: 3 },
] as const

const LOCKDOWN_CELL_CAPACITY = LOCKDOWN_CELL_AREAS.reduce((sum, cell) => sum + cell.capacity, 0)
...
export class JailRoutineSystem {
  ...
  private checkPhaseTimer(): void {
    if (this.routineCompleted) return

    const def = this.getPhaseDef(this.currentPhase)
    if (!def) return

    const elapsed = (Date.now() - this.phaseStartedAt) / 1000
    const isFinalPhase = this.currentPhase === 8

    // Phase 8 is the last phase (Recuento final) — no next phase to warn about.
    if (!isFinalPhase && !this.warningEmitted && elapsed >= def.duration - PHASE_WARNING_BEFORE_S) {
    ...
    if (elapsed >= def.duration) {
      if (isFinalPhase) {
        // End of last phase → routine completed. Game manager reads this and
        // ends the match (guards win — prisoners failed to escape in time).
        this.routineCompleted = true
        console.log('[JAIL] Routine complete — phase 8 (Recuento final) ended')
        return
      }
      this.advancePhase()
    }
  ...
  private buildPhaseAssignments(def: JailPhaseDef): NPCAssignment[] {
    ...
    if (def.phase === 8) {
      return this.buildCellAssignments(npcIds, def)
    }

    return this.buildStandardAssignments(npcIds, def)
  }

  private buildCellAssignments(npcIds: string[], def: JailPhaseDef): NPCAssignment[] {
    return npcIds.map(npcId => this.buildCellAssignment(npcId, def.actions))
  }
  ...
  private buildStandardAssignments(npcIds: string[], def: JailPhaseDef): NPCAssignment[] {
    ...
    for (const npcId of npcIds) {
      if (paired.has(npcId) || assigned.has(npcId)) continue
      ...
      if (subZone === 'celdas') {
        ...
        assignments.push(this.buildCellAssignment(npcId, actionPool, subZone, walkSpeedMult))
        assigned.add(npcId)
        continue
      }
      ...
      if (action.type === 'SOCIAL') {
        const partner = this.findPartner(npcIds, paired, assigned, npcId)
        if (partner) {
          ...
          assigned.add(npcId); assigned.add(partner)
   ...
   * Used for Work and Free Time when an NPC is assigned to lavanderia.
  ...
  private buildCellAssignment(
    npcId: string,
    actions: ActionDef[],
    subZone?: string,
    walkSpeedMult = 0.85 + Math.random() * 0.30,
  ): NPCAssignment {
    const actionPool = this.personality.applyWeightModifiers(npcId, actions)
    const action = this.weightedRandom(actionPool) ?? actions[0]
    if (!action) return this.buildIdleAssignment(npcId)

    const dur = this.randomDuration(action)
    const seed = generateSeed()
    const zoneId = this.getCellZoneForNPC(npcId)
  ...
  private checkReassignInterval(): void {
    ...
    for (const [npcId, timer] of this.npcTimers) {
      ...
      if (this.currentPhase === 8) {
        const assignment = this.buildCellAssignment(npcId, def.actions)
      ...
      if (subZone === 'celdas') {
        const walkSpeedMult = 0.85 + Math.random() * 0.30
        const assignment = this.buildCellAssignment(npcId, this.getActionPool(def, subZone), subZone, walkSpeedMult)
        this.npcAssignments.set(npcId, assignment)
        this.npcTimers.set(npcId, assignment.duration)
        changed.push(assignment)
        continue
  ...
  private findPartner(
    npcIds: string[],
    paired: Set<string>,
    assignedOrRequester: Set<string> | string,
    maybeRequesterId?: string
  ): string | null {
    const assigned = typeof assignedOrRequester === 'string' ? new Set<string>() : assignedOrRequester
    const requesterId = typeof assignedOrRequester === 'string' ? assignedOrRequester : maybeRequesterId
    if (!requesterId) return null

    const candidates = npcIds.filter(id => id !== requesterId && !paired.has(id) && !assigned.has(id))
  ...
  private assignCells(): void {
    ...
    npcIds.forEach((id, i) => {
      this.npcCells.set(id, this.getCellZoneForSlot(i))
    })
  }

  private getCellZoneForNPC(npcId: string): string {
    const existing = this.npcCells.get(npcId)
    if (existing) return existing

    const npcIds = Array.from(this.state.npcs.keys()).sort()
    const index = Math.max(0, npcIds.indexOf(npcId))
    const cellZoneId = this.getCellZoneForSlot(index)
    this.npcCells.set(npcId, cellZoneId)
    return cellZoneId
  }

  private getCellZoneForSlot(slotIndex: number): string {
    const bedSlot = ((slotIndex % LOCKDOWN_CELL_CAPACITY) + LOCKDOWN_CELL_CAPACITY) % LOCKDOWN_CELL_CAPACITY
    let cursor = 0

    for (const cell of LOCKDOWN_CELL_AREAS) {
      cursor += cell.capacity
      if (bedSlot < cursor) return cell.zoneId
    }

    return LOCKDOWN_CELL_AREAS[0].zoneId
  }
