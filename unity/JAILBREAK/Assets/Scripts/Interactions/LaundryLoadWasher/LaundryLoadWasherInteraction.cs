...
/// <summary>
/// Character-side component that drives the looping "loading washer" animation while
/// the character is stationed at a <see cref="LaundryLoadWasherInteractable"/>'s
/// action point.
///
/// Mirrors the authoring pattern of <see cref="WorkTableInteraction"/>: the state
/// lives on the character, the world-side interactable is used only as a position
/// lookup (via <see cref="Jailbreak.NPC.ZoneRegistry"/>).
///
/// Also handles the hand-prop swap for NPCs (destroying unfolded clothes on start,
/// attaching folded clothes on completion), mirroring the local player flow.
/// </summary>
public class LaundryLoadWasherInteraction : MonoBehaviour
{
    [Header("References")]
    ...
    [Header("Animator")]
    [Tooltip("Bool flipped on while loading the washer. Matches the ProgressAction on the world-side washer (default: isLoadingWasher).")]
    public string animatorBoolName = "isLoadingWasher";
    ...
    void StartWork(Transform actionPoint)
    {
        ...
        // 4) Silently remove the unfolded clothes bundle (like the player does)
        var clothes = GetComponent<CarryClothesInteraction>();
        if (clothes != null)
        {
            clothes.ForceReset();
        }

        // 5) Drive the animator
        if (animator != null)
        {
    ...
    void StopWork()
    {
        ...
        // Attach the folded clothes when done
